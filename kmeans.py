...
def estep(X: np.ndarray, mixture: GaussianMixture) -> np.ndarray:
    ...
    for i in range(n):
        mask= X[i]!=0
        sse = ((X[i,mask] - mixture.mu[:,mask])**2).sum(axis=1)
        j = np.argmin(sse)
        post[i, j] = 1
...
def mstep(X: np.ndarray, post: np.ndarray, mixture: GaussianMixture) -> Tuple[GaussianMixture, float]:
...
def run(X: np.ndarray, mixture: GaussianMixture,
    ...
    while (prev_cost is None or prev_cost - cost > 1e-4):
        ...
        mixture, cost = mstep(X, post, mixture)
